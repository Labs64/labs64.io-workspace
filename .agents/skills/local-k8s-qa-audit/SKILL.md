...
One Markdown report at `.agents/reports/`, named `QA_AUDIT_REPORT_YYYYMMDD.md` (today's date). If a file with the same name already exists for today, add an incremental suffix (e.g., `QA_AUDIT_REPORT_YYYYMMDD_1.md`) so successive audits don't overwrite each other. The report should contain these sections in order:
